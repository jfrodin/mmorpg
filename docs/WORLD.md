...
- **v0.1 startområde** (klient-sida tile-karta, `client/src/world/starting-area.ts`, 170×110 rutor): ett litet torg (kullersten) med fem enkla byggnadsplaceringar runt om, en väg österut ut ur torget, ett skogsområde västerut (strödda träd, ~16% densitet), en bit sjöstrand söderut (sand → vatten), ute på sjön en liten obebodd ö (skog, ~12% träddensitet) och en mindre rent visuell holme, samt vägen österut leder vidare till en liten lantlig korsning med en gård (bostadshus + lada, oidentifierad ägare) och en NPC (Ingvar).
